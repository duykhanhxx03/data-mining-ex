...
\author{Họ và tên: \rule{5cm}{0.4pt}\\
        MSSV: \rule{3.5cm}{0.4pt}}
\date{\today}
...
và CBF cho kết quả truy vấn \emph{trùng nhau từng khóa}; (ii) payload của
CBF 4-bit tốn đúng $4\times$ BF, do đó với cùng ngân sách byte, FPR của CBF
(\EqFprCBFFour{}) cao hơn BF (\EqFprBF{}) khoảng \EqRatioFour{} lần,
tức hơn một bậc độ lớn; (iii) CBF hỗ trợ
xóa an toàn khi caller chỉ xóa khóa chắc chắn đã chèn: xóa đúng
\FnValidDeleted{} khóa không làm mất khóa thật nào còn lại, trong khi stress
test cố ý xóa các false positive tạo ra \FnCount{} false negative
(\FnRate{}). Kết luận: dùng BF cho tập gần như tĩnh; dùng CBF khi cần xóa động
...
cầu caller chỉ xóa khóa chắc chắn đã chèn: câu trả lời ``có thể có'' của Query
...
xác suất này ``minuscule'', counter 4 bit (đếm $0..15$) đủ cho hầu hết ứng
dụng. CBF tốn $4\times$ payload so với BF cùng số ô~\cite{broder2004}.
...
theo dõi overflow hoặc rebuild từ nguồn dữ liệu chuẩn.
...
biết khóa đã chèn và không thêm Query; riêng E4 dùng một hàm có Query guard để
...
FPR được báo cùng khoảng tin cậy Wilson 95\%. Các phép đo throughput lặp
...
\begin{itemize}
  \item[\textbf{E1}] FPR thực nghiệm vs.\ lý thuyết (kèm KTC 95\%) và
        throughput median/IQR của insert/query/delete theo $m/n$.
...
throughput BF $\ge$ CBF cho các thao tác chung.
...
\label{tab:fpr}
\resizebox{\textwidth}{!}{\input{tab/fpr.tex}}
\end{table}
...
và tỉ số phản ánh cụ thể hiện thực \texttt{bitarray}/\texttt{bytearray}, thao
tác nibble và vòng lặp Python; chúng không chứng minh mọi cài đặt BF luôn nhanh
hơn mọi cài đặt CBF.
...
Phần hai cố ý vi phạm contract của delete: hệ thống nhầm câu trả lời ``có thể
...
(\FnRate{}) biến mất khỏi bộ lọc. Đây là kết quả có điều kiện của stress test
với hàng nghìn yêu cầu xóa sai, \emph{không phải} FNR mặc định của CBF và không
được tổng quát hóa sang workload dùng đúng. Biện pháp chính là chỉ xóa khóa
...
xuống $\sim$1, đổi lấy FPR tăng nhẹ; RocksDB dùng biến thể cache-line-local
trong full filter~\cite{rocksdbBloom}; không hỗ trợ xóa.
\textbf{Cuckoo Filter}~\cite{cuckoo2014}: lưu fingerprint trong
...
4-bit. Đây là một lựa chọn thay thế CBF tốt trên phần cứng.
...
\begin{enumerate}
  ...
  \item \textbf{Cần xóa động hoặc đếm bội}:\\ dùng \textbf{CBF 4-bit} khi hệ
        thống có nguồn sự thật để bảo đảm chỉ xóa khóa đã chèn. Theo dõi
        overflow; rebuild từ nguồn chuẩn khi counter mất thông tin hoặc
...
tiểu theo $k$ tại $k^*$ (H4). Bức tranh tổng thể: \emph{CBF mua khả năng xóa
bằng payload counter lớn hơn và một contract delete chặt hơn}. Khi chỉ xóa
...
khi có nguồn sự thật cho delete; và Cuckoo Filter đáng khảo sát khi cần xóa
với yêu cầu FPR/bộ nhớ chặt.

\paragraph{Hạn chế.} Benchmark dùng một hiện thực Python cụ thể; dù đã báo
median/IQR qua \BenchRepeats{} lượt, cả số tuyệt đối lẫn tỉ số không đại diện
cho mọi hiện thực C/C++/phần cứng. FPR dùng một tập khóa tất định và một cặp
...
có khóa. Các số bộ nhớ chỉ là payload backing array, không phải RSS.
